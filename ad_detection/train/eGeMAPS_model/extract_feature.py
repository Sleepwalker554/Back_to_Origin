...
class CsvDataset(Dataset):
    ...
    def __init__(self, csv_path: Path, raw_audio_dir: Path):
        ...
        # Feature paths in CSV are relative to the CSV's directory
        # (matches dataset.py resolution convention)
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                self.data.append({
                    ...
                    'audio_path_abs': str(audio_path),
                    'feature_path_abs': str(feature_path_abs),
                })
    ...
    def __getitem__(self, index):
        ...
        return item['audio_path_abs'], item['feature_path_abs'], item['session_id']
...
def extract_egemaps_features_from_csv(csv_path: Path, raw_audio_dir: Path):
    ...
    for audio_path_abs, feature_path_abs, session_id in tqdm(dataloader, desc="Extracting"):
        audio_path_abs = Path(audio_path_abs)
        feature_path_abs = Path(feature_path_abs)

        if feature_path_abs.exists():
            skipped += 1
            continue
        ...
        try:
            ...
            # Drop trailing samples that don't fill a full segment
            ...
            feature_path_abs.parent.mkdir(parents=True, exist_ok=True)

            torch.save(egemaps, feature_path_abs)
